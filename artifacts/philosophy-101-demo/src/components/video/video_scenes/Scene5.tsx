...
export function Scene5({ setCursorPos, setIsClicking }: { setCursorPos: (pos: {x: string, y: string}) => void, setIsClicking: (val: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const t9 = setTimeout(() => {
      ...
      setTypedAnswer("Sound");
    }, 8500);
    const t10 = setTimeout(() => setTypedAnswer("Sound — a valid"), 9000);
    const t11 = setTimeout(() => setTypedAnswer("Sound — a valid argument with"), 9500);
    const t12 = setTimeout(() => setTypedAnswer("Sound — a valid argument with all true premises."), 10000);
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          <div className="p-8 flex-1 border-b border-border text-lg text-primary font-medium leading-relaxed">
            {phase >= 1 && phase < 4 && (
              <StreamingText text="Does a valid argument guarantee a true conclusion? Answer yes or no, and explain." delay={0} />
            )}
            {phase >= 4 && (
              <StreamingText text="What do we call a valid argument with all true premises?" delay={0} />
            )}
          </div>
          ...
          <div className="p-6 bg-muted/20">
              ...
              {phase >= 2 && phase < 4 && (
                ...
                >
                  ...
                  <div className="text-sm text-red-900 leading-relaxed">
                    <StreamingText text="Not quite. A valid argument guarantees a true conclusion only if its premises are also true — that combination is soundness. Validity alone is just about the argument's form." delay={0} />
                  </div>
                </motion.div>
              )}
              {phase >= 5 && (
                ...
                >
                  ...
                  <div className="text-sm text-emerald-900 leading-relaxed">
                    <StreamingText text="Correct. A valid argument with all true premises is sound — and a sound argument's conclusion must be true." delay={0} />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="mt-4">
              <div className="flex space-x-2 border-b border-border pb-2 mb-3 overflow-x-auto text-xs font-medium">
                <div className="px-3 py-1 border-b-2 border-primary text-primary">Logic</div>
                {['Fallacies', 'Reconstruction', 'Epistemology', 'Skepticism', 'Mind', 'Free Will', 'Metaphysics'].map(t => (
