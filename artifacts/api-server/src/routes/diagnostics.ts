    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single college philosophy practice problem on "${topic.title}" at easy difficulty, with a short answer (a word, term, "yes"/"no", or short phrase). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
  ...
  for (const p of sample) {
    steps.push(
      ...
      await run(
        ...
        async () => {
          // The course teaches a specific framework and terminology that can
          // differ from how a given topic is presented elsewhere. Both phases are
          // grounded in THIS lecture so the check judges keys against what the
          // course actually teaches — not generic textbook philosophy.
          ...
          const derived = await chatText(
            "You are a strong college philosophy student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for a college philosophy course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are a college philosophy lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
